...
$(function() {
	...
	*/
	function moveRight () {
		...
		var cnt = 0;
		$ps_albums.children('div:nth-child('+first+')').animate({'left': - hiddenLeft + 'px','opacity':0},0,function(){
				var $this = $(this);
				$ps_albums.children('div').slice(first,parseInt(first+2)).each(
					function(i){
						var $elem = $(this);
						$elem.animate({'left': positions[i] + 'px'},300,function(){
							++cnt;
							if(cnt == 2){
								$ps_albums.children('div:nth-child('+parseInt(first+3)+')').animate({'left': positions[cnt] + 'px','opacity':1},0,function(){
									//$this.hide();
									++first;
	...
	*/
	function moveLeft () {
		...
		var cnt = 0;
		var last= first+2;
		$ps_albums.children('div:nth-child('+last+')').animate({'left': hiddenRight + 'px','opacity':0},0,function(){
				var $this = $(this);
				$ps_albums.children('div').slice(parseInt(last-3),parseInt(last-1)).each(
					function(i){
						var $elem = $(this);
						$elem.animate({'left': positions[i+1] + 'px'},300,function(){
							++cnt;
							if(cnt == 2){
								$ps_albums.children('div:nth-child('+parseInt(last-3)+')').animate({'left': positions[0] + 'px','opacity':1},0,function(){
									//$this.hide();
									--first;
									enableNavRight();
									if(first > 1){
